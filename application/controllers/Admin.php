...
class Admin extends CI_Controller
{
  ...
  public function index()
  {
    if ($this->session->userdata('status') == 'login' && $this->session->userdata('role') == 1) {
      ...
      $data['stokBarangMasuk'] = $this->M_admin->sum('tb_barang_masuk', 'jumlah_masuk');
      $data['stokBarangKeluar'] = $this->M_admin->sum('tb_barang_keluar', 'jumlah_keluar');
  ...
  {
    $data['list_satuan'] = $this->M_admin->select_form('tb_satuan');
    $data['token_generate'] = $this->token_generate();
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
    $this->session->set_userdata($data);
  ...
  {
    $data = array(
      'list_data' => $this->M_admin->select_barang_masuk('tb_barang_masuk'),
  ...
  public function delete_barang($id_transaksi)
  {
    ...
    $get_image_file = $this->db->get('tb_barang_masuk')->row();
    @unlink('assets/images/' . $get_image_file->qr_code);
    $this->db->where('id_transaksi', $id_transaksi);
  ...
  public function proses_databarang_masuk_insert()
  {
    ...
    if ($this->form_validation->run() == TRUE) {
      ...
      $jumlah       = $this->input->post('jumlah_masuk', TRUE);
      ...
      $params['data'] = "Kode Barang :" . $kode_barang . " Nama Barang :" . $nama_barang; //data yang akan di jadikan QR CODE
  ...
  {
    $data['list_data'] = $this->M_admin->select_form('tb_satuan');
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
  ...
  public function laporan_masuk()
  {
    ...
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
    $this->load->view('admin/Laporan/laporan_masuk', $data);
  }
  ####################################
  // End Laporan DATA BARANG MASUK
  ####################################
  ...
  public function proses_data_keluar()
  {
    ...
    if ($this->form_validation->run() === TRUE) {
      ...
      $nama_barang    = $this->input->post('nama_barang', TRUE);
      $satuan         = $this->input->post('satuan', TRUE);
      $jumlah_keluar  = $this->input->post('jumlah_masuk', TRUE);

      $image_name = $kode_barang . '.png';
  ...
  {
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
    $this->load->view('admin/qrcode/v_qrcode', $data);
  }

  public function cetak_qrcode()
  {
    $data = array(
      'konten' => $this->db->get('tb_barang_masuk')->result_array(),
    );
    $this->load->view('admin/qrcode/c_qrcode', $data);
  }
}
